...
def get_parent_hidden(r_file, match='Y'):
    with open(r_file, 'r') as f:
        all_hidden = []
        for i, line in enumerate(f):
            data = json.loads(line)
...
def get_child_hidden(r_file, match='Y'):
    with open(r_file, 'r') as f:
        all_hidden = []
        for i, line in enumerate(f):
            data = json.loads(line)
...
def plot_pca(model, r1_file, r2_file):
    import matplotlib.pyplot as plt
    from sklearn.decomposition import PCA

    # 将数据合并在一起用于PCA
    group1 = get_parent_hidden(r1_file, match='Y')
    group2 = get_child_hidden(r2_file, match='N')
    ...
    plt.savefig('../figure/celebrity_pca_{}.pdf'.format(model), dpi=300, bbox_inches='tight')
    # plt.show()
...
if __name__ == "__main__":
    ...
    for model_name_or_path in MODELS:
        ...
        # r_file =  '../data/term_out_{}.jsonl'.format(model_name_or_path)
        # print('=========== {} ==========='.format(r_file), '\n')
        # plot_overlapping_density_euclidean(model_name_or_path.split('/')[-1], r_file)

        r1_file='../data/celebrity_out_parents_{}.json'.format(model_name_or_path)
        r2_file='../data/celebrity_out_child_{}.json'.format(model_name_or_path)
        plot_pca(model_name_or_path.split('/')[-1], r1_file, r2_file)
