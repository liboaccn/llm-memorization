...
def create_prompt_ground(question, answer, prompt_num=6):
    from load_LLMs import CRYTONITE_PROMPT_10

    if prompt_num == 10:
        prompt = CRYTONITE_PROMPT_10.format(question)
    else:
        prompt = question
...
def generate_cryptonite(r_file, w_file, model, tokenizer, prompt_num=6):
...
if __name__ == '__main__':
    ...
    r_file = '../data/cryptonite.jsonl'

    for prompt_num in [10]:
        for model_name_or_path in MODELS:
            logging.info('Loading model: {}'.format(model_name_or_path))

            w_file = '../data/cryptonite_out_{}_shot_{}.jsonl'.format(prompt_num, model_name_or_path.split('/')[-1])
            logging.info('written file: {}'.format(w_file))

            model, tokenizer = load_model(model_name_or_path)
            generate_cryptonite(r_file, w_file, model, tokenizer, prompt_num)
