...
def plot_overlapping_density_euclidean(model, r_file):
    ...
    plt.savefig('../figure/idiom_overlapping_density_hidden_{}.pdf'.format(model), dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_pca(model, r_file):
    ...
    plt.savefig('../figure/peotry_pca_{}.pdf'.format(model), dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    for model_name_or_path in MODELS:
        # r_file = '../data/shi_out_next_{}_shot_{}.jsonl'.format(0, model_name_or_path.split('/')[-1])
        r_file = '../data/shi_out_{}_last_{}.jsonl'.format(model_name_or_path.split('/')[-1], 1)
        print('=========== {} ==========='.format(r_file), '\n')
