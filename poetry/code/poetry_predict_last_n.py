...
def create_prompt_ground(previous_line, next_line, last_n):
    # 句子内部
    prompt = '' + previous_line[:-last_n]
    ground_truth = previous_line[-last_n:]

    return prompt, ground_truth


def generate_poetry(r_file, w_file, model, tokenizer, last_n=1):
    ...
    with open(r_file, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            prompt, ground_truth = create_prompt_ground(previous_line, next_line, last_n)
            max_gen_tokens = last_n
            ...
            print("original: ", previous_line)
            print("Generated: ", generated_text[-max_gen_tokens:], '\n')
            ...
            data = {
                ...
                'generated_text': generated_text[-max_gen_tokens:],
...
if __name__ == '__main__':
    ...
    for model_name_or_path in MODELS:
        ...
        w_file = '../data/shi_out_{}_last_{}.jsonl'.format(model_name_or_path.split('/')[-1], last_n)
        logging.info('written file: {}'.format(w_file))

        model, tokenizer = load_model(model_name_or_path)
        generate_poetry(r_file, w_file, model, tokenizer, last_n)
